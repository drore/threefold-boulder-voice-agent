...
The reasoning backend uses distinct `lookupMunicipalCode`, `lookupCityInformation`, and `lookupCityWebsite` tools; the website tool discovers an official page from the city sitemap by keyword, fetches it live, and answers only from the extracted text with its URL, failing closed when the sitemap or page is unavailable. The current interview slice serves three checked-in reviewed examples directly through these tools; `KnowledgeProvider`, `retrieveEvidence`, and a broader corpus are later design seams, not prerequisites to demonstrate the assignment. The distinct `findCityEvents` tool accepts a local-date range and uses trusted server time. It serves one checked event with a freshness cutoff. No source is trusted merely because the model requested it.
