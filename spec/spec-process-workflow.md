...
`CityConfig` (server DB data): city ID, revision, timezone, schedule source URL/verifiedAt/validThrough, weekly opening hours, verified date overrides, department records, requestType -> department mapping, and `alwaysOpenTicket`. The configuration adapter validates timezone, source metadata, opening hours ordered without overlaps within a day, supported department mappings, mock destinations, and validity horizon, and matches city identity to the authorized input. The first pure policy slice checks validated hours independently, then maps open/closed/indeterminate to route/ticket/unavailable; department/destination resolution belongs to workflow composition. P0 reads before each action; caching is deferred.
...
- AC-009: Given a complete pothole draft and an explicit browser confirmation of its current revision during office hours, the server reloads that draft and the DB policy, then displays an honestly labeled simulated route to Transportation's configured mock number. A stale revision or unavailable policy produces no route. Outside office hours, a configured Linear adapter authorizes one durable create attempt and reads the returned issue before claiming creation; absent configuration, it says no ticket was created. Repeating a confirmation fetches an existing ticket even after hours change. An uncertain attempt never triggers another create automatically.
- AC-010: Given a complete park-maintenance draft, the same core decision maps open hours to Parks & Recreation's configured mock destination and closed hours to a ticket requirement. The local Postgres operation retains the park report type and details; browser/voice and live Linear evidence are separate integration gates.
