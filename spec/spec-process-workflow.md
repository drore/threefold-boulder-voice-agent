...
The implemented first intake slice prepares a pothole draft only. A complete draft returns a revision-bound summary for a later confirmation request; this is not yet a persisted confirmation prompt or an accepted request. Missing location or description returns `needs_input`. Corrections create a new revision, so any summary for an earlier revision is stale. The offline fake and local Postgres adapter check scoped observation references and expected revisions; confirmation and deployment-ready session handling remain to be implemented.
