...
The implemented `prepareServiceReport` slice covers potholes and park maintenance. Its server-supplied context contains conversation, city, and admission IDs. The server also selects a draft ID and expected revision: both are `null` to create a draft, or both identify the current draft and integer revision to update it. This does not decide COR-P01's proposed session-wide draft limit. The saved draft explicitly records the selected supported request type. Candidate location and description each carry text and a server-issued observation reference. The draft store checks admission/conversation scope and observation ownership, and atomically compares the expected revision when saving. The core merges an observed patch with the existing draft, trims bounded text, leaves the revision unchanged when no new fields are proposed, and returns missing required fields or a summary bound to that revision. `needs_confirmation` here means the details are complete enough to request confirmation; it does **not** by itself establish caller acceptance or authorize a ticket/transfer. The in-memory test store verifies this contract offline; real local Postgres tests verify scope, revision races, and persistence across store instances. The loopback UI/API now admits each browser visitor into a separate conversation and holds its active draft pointer in process memory. A Node restart loses that pointer, so draft recovery across restarts is not yet proven.
