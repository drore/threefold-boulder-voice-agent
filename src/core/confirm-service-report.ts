...
type RoutePolicy = Readonly<{
  ...
  requestTypes: Readonly<Record<SupportedReportType, string>>;
...
export type ConfirmServiceReportDecision =
  | {
 ...
 */
export async function confirmServiceReport(
  ...
  const departmentId = policy.requestTypes[draft.requestType];
  const department = departmentId && policy.departments[departmentId];
