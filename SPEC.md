...
Status: local text intake and confirmation now cover potholes and park maintenance, with distinct mock destinations selected from the DB-backed Boulder policy. Closed-hours reports use a durable one-attempt Linear operation when the demo project and API key are configured; an owner-reported live create/readback on September 16, 2026 (issue DRO-5 in the dedicated demo project, verified through the Linear API; not yet captured as a reproducible in-repo artifact). Reviewed code and service answers work locally, and event answers now come from a live cached fetch of the official Boulder calendar. A "what can you do" turn answers with a fixed options overview. Browser GPT-Live session and client-delegation code is connected to the application tools; Dror personally exercised the spoken browser path on September 16 with a positive report, while formal recorded voice evidence remains pending. Each browser visitor has a separate server-owned conversation, draft pointer, and usage quota; reviewer mode requires a code and secure cookie but has not been deployed or browser-verified. The reviewer deployment is live (code-gated; see EVIDENCE D2), while formal voice evidence and the final writeup review remain open. Dror authorized the project and use of the existing OpenAI development key; submission to Threefold remains Dror's decision.
...
The [assignment](https://www.threefold.ai/developer-task) calls this a small version of Threefold's work and explicitly permits partial completion. Assignment coverage has priority over additional features; any cut is reported honestly.
...
| --- | --- | --- | --- |
| V0 | Voice agent for a US city | Fresh reviewer speaks/hears relevant responses through the delivered link; Boulder and demo limitations are clear. | Not passed — browser WebRTC and client delegation are implemented and fake-tested, but there is no recorded audio; formal recorded/deployed voice proof pending (requires a recorded voice run) |
| R1 | Answer municipal-code and website questions | Separate spoken examples use actual code and official service guidance, preserving qualifications and exposing relevant sources. Website summaries alone do not establish code coverage. | Reviewed code/service answers work locally in text; live reasoning tool selection passed; spoken answers pending |
| R2 | Answer current city-event questions | Dated official news/events support current answers, including freshness, past/upcoming status, times, and cancellations. | Live cached fetch of the official Boulder calendar answers upcoming events with dated entries and official links; 19 occurrences parsed on 2026-09-16; voice proof pending |
| R3 | Open a ticket in a selected platform | Confirmed spoken report creates a real Linear issue in the approved demo project; a real API readback verifies ID and fields. | Owner-reported live result on 2026-09-16 (DRO-5): issue created and independently read back with matching team/project/title/description; not yet captured as a reproducible in-repo artifact; spoken trigger pending |
...
| R5 | Deterministically route or ticket according to city hours | DB-backed schedules and server code determine actions; repeatable boundary/timezone/closure tests and spoken open/closed scenarios prove enforcement. | Local open/closed and two-department paths pass; closed-hours DRO-5 create/readback owner-reported on 2026-09-16; spoken proof pending |
| R6 | Evaluation/testing setup | Runnable commands, versioned cases/results, and meaningful failures cover R1–R5. Application, provider, model, and voice verification are distinguished. | 195 local DB tests pass (21 files); eleven opt-in live reasoning cases recorded 18/22 (82%) on 2026-09-17 with three tool-selection failures; credential-free CI workflow passed on the public repository 2026-09-17; spoken and delivery evidence pending |
...
| D2 | Link reviewers can try | Fresh session verifies the deployed revision's voice, sources, real demo tickets, and simulated routing; access steps documented. | Deployed 2026-09-19 at `https://threefold-boulder-agent.onrender.com`, code-gated; health, UI, sourced answers, and live events verified; voice by a fresh reviewer pending |
...
| J3 — information | Reviewed BRC 8-3-9 glass-container rule, official pothole-reporting guidance, and upcoming events fetched live from the official Boulder calendar. | No reservations/payments, city-wide legal coverage, or unsupported precise claims. Event answers defer times/cancellations to official detail pages. Broader source coverage remains a later extension. |
...
First external TicketProvider is Linear, targeting a dedicated demo project in Dror's Linear workspace. Dror explicitly authorized creating that project and using synthetic tickets there. The project was created on September 16, 2026, and Dror reported a live create/readback the same day (issue DRO-5): the app's closed-hours confirmation flow created the issue and an independent Linear API readback matched ID, title, description, team, and project. This is owner-reported and not yet captured as a reproducible in-repo artifact. The adapter requires both team and project IDs, so issues cannot silently land in the team's general backlog. Readback accepts an issue only when its returned ID, identifier, title, and description match the created issue, and creation was enforced into the configured team and project; the receipt keeps Linear's human identifier as the caller-facing reference while the internal UUID stays server-side. A moved or mismatched issue cannot count as a verified receipt. In approved Linear integration and the deployed demo, applicable confirmed requests create actual Linear issues; verification, reconciliation, and needed authorized ticket detail/status reads query the real Linear API. Linear owns the external ticket and its current provider state; Supabase owns conversation/workflow history, references, receipts, and explicitly timestamped read snapshots. Local records or generated mockup IDs cannot substitute for a current Linear read. Reads are scoped to server-authorized linked tickets; general workspace browsing and cross-conversation ticket discovery are outside P0.
...
Client delegation is selected for ownership of reasoning context, validation, and later model comparison. The application assembles scoped transcript/task context, handles delegation events, runs the reasoning workflow, and returns verified concise results to GPT-Live. The first reasoning proposal model is `gpt-5.6-luna` with strict structured output; local key access and two live synthetic classifications succeeded, but broader accuracy/cost remains an evaluation gate. Browser microphone/media and actual delegation ordering still need empirical verification. The current local voice journey treats spoken confirmation as sufficient for the voice channel (via the server-owned, revision-bound `confirmReport` tool), while the text channel keeps on-screen confirmation; action results are then appended to the report's voice delegation. This mode does not guarantee every spoken word. Mandatory exact speech needs controlled playback, outside P0.
...
The P0 reasoning backend has five agent-facing capabilities: municipal-code lookup, city service/department information lookup, live official-website page lookup, dated city-event search, and preparation of a supported service report. The website lookup discovers the most relevant official page from the city sitemap by matching the caller's keywords, fetches it on demand, and answers only from the extracted page text with its URL; the URL index and page text are cached with a TTL and the lookup fails closed. Event search is separate from general city information because date range, timezone, current status, and freshness determine its answers. `isWithinBusinessHours` and action mapping remain internal core policy, never agent tools. The service-report tool prepares a durable local pothole or park draft; the separate local browser confirmation path reads the current draft and DB policy to simulate a route or authorize one durable Linear create/read attempt. Repeat confirmation fetches a linked issue before re-evaluating hours, and a known issue ID can be reconciled by read-only lookup after an uncertain first readback. Municipal-code and city-information capabilities answer one reviewed example each and return limited coverage for unsupported questions; the website capability is dynamic over the official sitemap and fails closed; event search answers from the live cached official calendar. A delegated turn now runs one bounded tool-calling reasoning turn: the model chooses the application tools, the server executes them with validated scope, and the model composes a short grounded reply, so capability questions are answered conversationally from the tool list rather than a fixed message. Live reasoning-classification calls passed and one real Linear ticket was created and verified, but no full spoken journey or phone transfer has been formally recorded, so R1–R5 voice evidence is not yet complete.
...
| Q2 | One HTTPS Node runtime candidate serves the built UI and same-origin API; browser WebRTC connects directly to OpenAI, so the server needs no WebSocket. Render is a candidate. Choose vendor/region/plan and cost before deployment. | M6 external approval |
| Q3 | Implemented local design: automatic per-visitor admission in development; reviewer code, opaque HttpOnly/Secure/SameSite cookie, exact Origin check on writes, separate conversation/draft/quotas in reviewer mode. Decide secure code delivery and verify hosted behavior before publishing. One Node process holds the session map. | Security review; M6 |
...
- [Simple responsive interface concept](design/README.md) — selected P0 direction; the single-screen demo UI is implemented.
...
- [Events](https://bouldercolorado.gov/events): the live provider fetches and parses this official dated listing (24-hour cache, fails closed on expiry + fetch failure); exercised 2026-09-16. [News](https://bouldercolorado.gov/news): dated listings checked September 15; details/refresh still to implement.
