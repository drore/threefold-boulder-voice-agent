...
Status: the local browser text path saves and confirms a pothole draft, reads the seeded Boulder policy from Supabase Postgres, and displays an honest simulated open-hours route. During closed hours it uses a durable one-attempt operation to create and read back a Linear issue when the dedicated demo project and API key are configured; without them it reports that no ticket was created. This full path has only been tested against a local provider fixture, not live Linear. Three reviewed information examples work in text. The dedicated Linear project exists, but its API connection needs reauthentication. Voice, live ticket proof, a second department route, and deployment remain unimplemented. Accepted principles, proposed defaults, and release prerequisites are distinguished below. Dror authorized the dedicated Linear project and use of the existing OpenAI development key; submission to Threefold remains Dror's decision.
...
| R1 | Answer municipal-code and website questions | Separate spoken examples use actual code and official service guidance, preserving qualifications and exposing relevant sources. Website summaries alone do not establish code coverage. | One reviewed code and one service answer work locally in text; voice pending |
| R2 | Answer current city-event questions | Dated official news/events support current answers, including freshness, past/upcoming status, times, and cancellations. | One reviewed event works locally in text until its freshness horizon; voice and refresh pending |
