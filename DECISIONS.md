...
**Status:** TypeScript/React/Node selected; local and reviewer runtime composition implemented, hosted verification pending. **SPEC:** ADR-011, ADR-016; runtime spec.
...
The hosted UI needs safe static-file serving. `@fastify/static` 10.1.3 is used for that narrow job rather than writing path handling ourselves; it supports Fastify 5 and is newer than the [10.1.2 fix for non-canonical path authorization](https://github.com/fastify/fastify-static/security/advisories/GHSA-8pvw-jcv7-9cmj). Reviewer mode registers the tested module against the explicit build directory. The service still needs hosted database, credentials, HTTPS, and a live browser check before it is deliverable.
...
**Why:** Authentic coherent commits show how the system grew and make review/regression diagnosis practical. A behavior, its tests, and its SPEC changes travel together in a usable milestone snapshot. Threefold permits partial completion, so evidence of a working reviewer path has higher value than platform breadth. Completing assignment evidence before optional tone, representative view, ticket-always execution, or shadow runners protects the review path.
...
**Tradeoff:** Commit boundaries need judgment and snapshot validation, especially with partial staging. Git revert does not undo a Linear ticket or database effects.

**Reconsider when:** Two changes cannot safely stand alone, or implementation evidence changes sequencing. Keep meaningful history and report limitations. Use the development plan's commit/recovery rules.
