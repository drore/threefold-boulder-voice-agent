...
Status: the local React form saves and confirms pothole and park maintenance drafts; the server reads DB-backed Boulder policy and displays a distinct simulated department route when open or uses a durable one-attempt Linear ticket path when closed and configured. Three reviewed code/service/event examples answer through the tool boundary and local browser. Browser GPT-Live WebRTC, client delegation, and a structured reasoning proposal are wired; two live synthetic reasoning calls passed, but microphone/spoken behavior has not been verified. The dedicated Linear project exists, and Dror reported a live create/readback on 2026-09-16 (issue DRO-5; owner-reported, not yet captured as a reproducible in-repo artifact). Public access/deployment, empirical voice evaluation, and delivery remain incomplete. [SPEC](SPEC.md) is the source of truth for behavior/scope; component specs own contracts. This plan owns task sequencing, evidence, and commits. Existing commands are documented in README; unimplemented commands below remain future targets.
...
Initial baseline: TypeScript/React/Node, one long-lived service, Supabase/Linear, GPT-Live with client delegation, and `gpt-5.6-luna` for bounded structured intent proposals. API availability and one synthetic reasoning result have been verified; the spoken journey remains an M1 check. Other material defaults in SPEC Q2–Q9 are reviewed before their corresponding release gate.
...
**Immediate working path:** local pothole and park intake, DB policy, two simulated open-hours routes, three reviewed text answers, closed-hours one-attempt ticket logic, and the voice delegation API pass local checks. Next exercise an actual spoken browser journey when microphone permission is available and verify one real synthetic issue in the dedicated Linear project after API access is restored. Then add compact empirical acceptance cases, public session admission, and reviewer deployment. Use an injected clock only in tests. Keep unavailable external paths explicit in the UI.
...
September 16 closed-hours checkpoint: the local API now authorizes a single durable ticket operation for a confirmed draft, calls the configured Linear provider once, verifies an issue by readback, and stores created/rejected/uncertain outcomes. Repeats read the existing issue before re-evaluating hours; known issue IDs can be reconciled by read-only lookup. The adapter requires a project ID for the dedicated Linear project. Independent review identified a runtime-role lock privilege bug and a correction/authorization race; both paths now lock the draft row, and the correction checks for an operation after acquiring that lock. `npm run check`, `npm run build`, and 107 local DB tests passed. The dedicated project was created, and Dror reported a live closed-hours DRO-5 create/readback on 2026-09-16 (owner-reported; not yet captured as a reproducible in-repo artifact). No Threefold submission has occurred.
