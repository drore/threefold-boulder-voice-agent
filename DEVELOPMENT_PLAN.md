...
Status: the local React form saves and confirms a pothole draft; the server reads the DB-backed Boulder policy and displays a simulated route when open or uses a durable one-attempt Linear ticket path when closed and configured. Three reviewed code/service/event examples answer through the tool boundary and local browser. The dedicated Linear project exists, but the API connection needs reauthentication; no live ticket has been verified. Voice, broader information and events, a second mock department, and delivery remain incomplete. [SPEC](SPEC.md) is the source of truth for behavior/scope; component specs own contracts. This plan owns task sequencing, evidence, and commits. Existing commands are documented in README; unimplemented commands below remain future targets.
...
**Immediate working path:** the local pothole intake, DB policy, simulated open-hours route, three reviewed text answers, and closed-hours one-attempt ticket path now pass local checks. Next verify one real synthetic issue in the dedicated Linear project after API access is restored, then add the second mock department and the smallest reviewer voice path. Use an injected clock only in tests. Keep unavailable external paths explicit in the UI.
